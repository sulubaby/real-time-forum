...
function authShell(title, subtitle, form, footer) {
    return `<main class="auth-page">
        <section class="auth-form-panel">
            <div class="auth-form-card"><p class="auth-wordmark">Real-Time Forum</p><p class="eyebrow">Welcome</p><h2>${title}</h2><p class="auth-subtitle">${subtitle}</p>${form}<p class="auth-switch">${footer}</p></div>
        </section>
    </main>`;
}
...
export function renderRegister(app, navigateTo) {
    app.innerHTML = authShell(
        ...
        `<form id="register-form" class="register-grid" novalidate>
            ...
            <div><label for="reg-firstname">First name</label><input type="text" id="reg-firstname" maxlength="15" autocomplete="given-name" placeholder="First name" required></div>
            <div><label for="reg-lastname">Last name</label><input type="text" id="reg-lastname" maxlength="15" autocomplete="family-name" placeholder="Last name" required></div>
